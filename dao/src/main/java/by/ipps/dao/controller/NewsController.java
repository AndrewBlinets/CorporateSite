...
public class NewsController extends BaseEntityAbstractController<News, NewsService>
  ...
  protected NewsController(
      NewsService newsService, ModelMapper modelMapper, LoggerService loggerService) {
    super(newsService, modelMapper, loggerService);
  ...
  public ResponseEntity<CustomPage<NewsDto>> getAllForClient(
    ...
    CustomPage<NewsDto> newsDto = this.modelMapper.map(news, targetListType);
  ...
  public ResponseEntity<NewsDtoFull> getByIdForClient(
    ...
    if (news != null) {
      NewsDtoFull newsF = this.modelMapper.map(news, NewsDtoFull.class);
      entityManager.unwrap(Session.class).disableFilter(FilterName.LANGUAGE);
  ...
  public ResponseEntity<CustomPage<NewsDtoAdmin>> getAllForAdmin(
    ...
    CustomPage<NewsDtoAdmin> newsDto = this.modelMapper.map(news, targetListType);
    entityManager.unwrap(Session.class).disableFilter(FilterName.LANGUAGE);
    return new ResponseEntity<>(newsDto, HttpStatus.OK);
  }
