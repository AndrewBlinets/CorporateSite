...
public abstract class BaseEntityAbstractController<
  ...
  protected BaseEntityAbstractController(
      S s, ModelMapper modelMapper, LoggerService loggerService) {
  ...
  @Override
  public ResponseEntity<List<T>> getAll(Sort sort) {
    List<T> ts = baseEntityService.findAll(sort);
    return new ResponseEntity<>(ts, ts != null ? HttpStatus.OK : HttpStatus.BAD_REQUEST);
  }
}
